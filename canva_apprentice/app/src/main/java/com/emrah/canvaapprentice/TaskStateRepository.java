...
public final class TaskStateRepository {
    private static final String PREFS = "agent_state_v1";
    ...
    public synchronized void markSafe(String hash) {
        ...
        prefs.edit().putString("last_safe_hash", hash == null ? "" : hash)
                .putInt("step", state.step + 1).apply();
    }

    public synchronized void pauseForHuman() {
        prefs.edit().putString("mode", TaskState.Mode.HUMAN_TAKEOVER.name()).apply();
    }

    public synchronized void resume() {
        prefs.edit().putString("mode", TaskState.Mode.RUNNING.name()).apply();
    }

    public synchronized void stop() {
        prefs.edit().putString("mode", TaskState.Mode.STOPPED.name()).apply();
    }
}
