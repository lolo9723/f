...
/**
 * Hazır modülleri TouchAgentServiceV2 yürütür; 20. modül ayrı AgentScriptRuntime'da çalışır.
 */
public class AgentAccessibilityService extends TouchAgentServiceV2 {
    ...
    private AgentScriptRuntime scriptRuntime;

    @Override
    protected void onServiceConnected() {
        super.onServiceConnected();
        scriptRuntime = new AgentScriptRuntime(this);
        AccessibilityServiceInfo info = getServiceInfo();
        info.packageNames = null;
        setServiceInfo(info);
        if (!AgentScriptEngine.selfTest()) {
            android.widget.Toast.makeText(this, "Agent Script öz testi başarısız. 20. modül devre dışı.", android.widget.Toast.LENGTH_LONG).show();
        }
    }
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        scriptRuntime.maybeHandleOwnAppClick(event);
    ...
    public void onDestroy() {
        if (scriptRuntime != null) scriptRuntime.destroy();
        super.onDestroy();
    }
    ...
    public static void finishLearning(Context c) {
        c.getSharedPreferences(STATE_PREF, Context.MODE_PRIVATE).edit()
                .putBoolean(KEY_LEARNING, false).remove(KEY_LEARNING_MODULE).remove(KEY_TARGET_PACKAGE).apply();
    }
