...
final class AgentScriptRuntimeV5 {
    ...
    private void vmListAppend(JSONObject s,int index){String n=s.optString("name");Object old=vm.get(n);JSONArray a=old instanceof JSONArray?new JSONArray(old.toString()):new JSONArray();a.put(vm.eval(s.opt("expr")));vm.set(n,a);persistVm();advance(index,40);}
